...
| `/etc/secrets/coolwsd.env` | `COLLABORA_AI_API_KEY` | Collabora (CODE) AI provider API key (Nous Portal), read by the `coolwsd` unit via `${...}` expansion — never in git or the nix store |
